<template>
	<div style="width: 100%;">
		<loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadMore" :auto-fill="autoFill">
			<div class="teacher-item" v-for="ne in china" @click="showDetail(ne.id, ne.name)">
...
export default {
  ...
  components: {
    ...
    showDetail(id, name) {
      ...
      params.append('submark', 1)
      params.append('index', this.china.length)
    	this.$common.http.post(this.$common.api.TeacherList, params)
          ...
          this.$store.commit(types.ADD_TEACHER_CHINA, {china: response.data})
    	  ...
    	  .catch(error => {
    	    ...
    	    this.$store.commit(types.ADD_TEACHER_CHINA, {china: china})
    	  });
	  },
    chinaDetail(chinaid) {
