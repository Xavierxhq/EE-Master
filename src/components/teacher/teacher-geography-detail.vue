<template>
  <transition name="fade">
    <div>
      <div class="title">
        <router-link class="router-link" to="/home/teacher/teacher-geography"><span class="glyphicon glyphicon-menu-left" aria-hidden="true"></span></router-link>
        <span>{{$route.params.geographyname}}</span>
      </div>
<!-- 详细内容 -->
...
export default {
  ...
  mounted() {
    this.fetchgeographyDetail()
  },
  methods: {
    fetchgeographyDetail() {
      this.$common.http.get(this.$common.api.TeacherInfo+'?teacher_id='+this.$route.params.geographyid)
        .then(response => {
        ...
        .catch(error => {
          ...
          this.detail.address = geographyDetail.address
          this.detail.careers = geographyDetail.careers
          this.detail.certifications = geographyDetail.certifications
          this.detail.evaluations = geographyDetail.evaluations
          this.detail.headimg = geographyDetail.headimg
          this.detail.intro = geographyDetail.intro
          this.detail.phone = geographyDetail.phone
          this.detail.submark = geographyDetail.submark
          this.detail.intro = geographyDetail.intro
          this.detail.submarkname = Convert.convertSubNumber(geographyDetail.submark)
          this.detail.gender = geographyDetail.gender
